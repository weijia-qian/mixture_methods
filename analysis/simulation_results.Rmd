...
# folder_path <- here("results", "wqs2_qg")
# 
# rda_files <- list.files(folder_path, pattern = "\\.RDA$", full.names = TRUE)
# length(rda_files)
# 
# # Robust loader: guarantees we extract object named `results`
# all_wqs2_qg <- vector("list", length(rda_files))
# for (i in seq_along(rda_files)) {
#   e <- new.env()
#   load(rda_files[[i]], envir = e)
#   stopifnot(exists("results", envir = e))
#   all_wqs2_qg[[i]] <- get("results", envir = e)
# }

folder_path <- here("results", "interactive")

rda_files <- list.files(folder_path, pattern = "\\.RDA$", full.names = TRUE)
length(rda_files)

# Robust loader: guarantees we extract object named `results`
all_interactive <- vector("list", length(rda_files))
for (i in seq_along(rda_files)) {
  e <- new.env()
  load(rda_files[[i]], envir = e)
  stopifnot(exists("results", envir = e))
  all_interactive[[i]] <- get("results", envir = e)
}
```
...
# all_coef_wqs2_qg <- list()
# all_weights_wqs2_qg <- list()
# 
# for (batch_res in all_wqs2_qg) {
#   for (iter_res in batch_res) {
#     if (is.null(iter_res)) next
#     if (!is.null(iter_res$coef))    all_coef_wqs2_qg[[length(all_coef_wqs2_qg) + 1]] <- iter_res$coef
#     if (!is.null(iter_res$weights)) all_weights_wqs2_qg[[length(all_weights_wqs2_qg) + 1]] <- iter_res$weights
#   }
# }

all_coef_int <- list()
all_weights_int <- list()
all_bkmr_int <- list()

for (batch_res in all_interactive) {
  for (iter_res in batch_res) {
    if (is.null(iter_res)) next
    if (!is.null(iter_res$coef))    all_coef_int[[length(all_coef_int) + 1]] <- iter_res$coef
    if (!is.null(iter_res$weights)) all_weights_int[[length(all_weights_int) + 1]] <- iter_res$weights
    if (!is.null(iter_res$bkmr))    all_bkmr_int[[length(all_bkmr_int) + 1]] <- iter_res$bkmr
  }
}
...
weights_df <- bind_rows(all_weights) %>%
  ...
  filter(scenario != "interactive") %>%
  bind_rows(all_weights_int) %>% 
  distinct() %>%
...
qg_df <- bind_rows(all_coef_qg)
# wqs2_qg_df <- bind_rows(all_coef_wqs2_qg)
int_df <- bind_rows(all_coef_int)

coef_df <- bind_rows(all_coef) %>%
  filter(!(method %in% c("qgcomp.noboot", "qgcomp.boot"))) %>%
  bind_rows(qg_df) %>% 
  filter(scenario != "interactive") %>%
  bind_rows(all_coef_int) %>% 
  # bind_rows(wqs2_qg_df) %>% 
  ...
  mutate(method = case_when(str_detect(method, "BKMR") ~ "BKMR",
                             ...
                             str_detect(method, "WQS2: pos") ~ "2iWQS:pos",
                             str_detect(method, "WQS2: neg") ~ "2iWQS:neg",
                             str_detect(method, "2iWQS") ~ "2iWQS",
                             .default = "WQS"),
         method = factor(method, levels = c("BKMR", "BWS", "qgcomp", "qgcomp nonlinear", "WQS", "2iWQS", "2iWQS:pos", "2iWQS:neg")),
         scenario = factor(scenario, levels = c("null", "single", "homogeneous", "heterogeneous", "nonlinear", "interactive")))

saveRDS(coef_df, here("results", "coef_df.rds"))
saveRDS(weights_df, here("results", "weights_df.rds"))
coef_df <- readRDS(here("results", "coef_df.rds"))
weights_df <- readRDS(here("results", "weights_df.rds"))
```
...
qgcomp_nonlinear_delta <- coef_df %>%
  filter(method == "qgcomp nonlinear" & term == "delta") %>%
  arrange(batch, seed) %>%
  select(batch, scenario, n, p, rho_X, sigma, seed, method, term, estimate, lb, ub) %>%
  select(-term)
...
df_power <- coef_df %>%
  filter(scenario != "null" & !(method %in% c("2iWQS","2iWQS:neg", "qgcomp nonlinear")) & term != "(intercept)") %>%
  mutate(reject0 = (lb > 0 | ub < 0)) %>%
...
collapse_rho <- function(x) {
  paste(sprintf("%.3f", x), collapse = " / ")
}

table2 <- perf_tbl %>%
  mutate(
    rho_X = factor(rho_X, levels = c(0, 0.4, 0.7))
  ) %>%
  arrange(scenario, method, rho_X) %>%
  group_by(scenario, method) %>%
  summarise(
    `Bias (ρ = 0 / 0.4 / 0.7)`     = collapse_rho(bias),
    `RMSE (ρ = 0 / 0.4 / 0.7)`     = collapse_rho(rmse),
    `Coverage (ρ = 0 / 0.4 / 0.7)` = paste(sprintf("%.2f", coverage), collapse = " / "),
    .groups = "drop"
  )
table2 %>%
  filter(scenario=="interactive") %>%
...
plot_se <- sens_spec %>%
  filter(scenario != "null" & sigma == 0.5) %>%
...
```{r}
df_aic <- coef_df %>%
  filter(!is.na(AIC_qgcomp)) %>%
  group_by(sigma, scenario, rho_X, seed) %>%
  summarize(favor_nonlinear = AIC_qgcomp[1] > AIC_qgcomp_ext[1], .groups = "drop") %>%
  group_by(sigma, scenario, rho_X) %>%
