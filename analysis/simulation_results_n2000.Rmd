...
```{r}
folder_path <- here("results", "n2000")
...
```{r}
df_type1 <- coef_df %>%
  filter(scenario == "null" & !method %in% c("qgcomp nonlinear", "2iWQS:neg", "2iWQS")) %>%
  mutate(reject0 = (lb > 0 | ub < 0)) %>%
  group_by(method, scenario, rho_X, sigma) %>%
  summarise(type1 = mean(reject0, na.rm = TRUE))
df_type1
...
df_power_wide <- df_power_wide %>%      
  select(BKMR, BWS, qgcomp, `qgcomp nonlinear: psi1`, `NA`, WQS, `2iWQS:pos`)
df_power_wide
