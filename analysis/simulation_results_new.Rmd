...
folder_path <- "/Users/weijia/Research/Huels/results/20250501"
...
df_weights$method <- factor(df_weights$method, levels = c("truth", "BKMR", "BWS", "qgcomp", "WQS"))
...
df_aic <- all_coef %>%
  ...
  summarize(favor_linear = AIC_qgcomp[1] < AIC_qgcomp_boot[1], .groups = "drop") %>%
  group_by(scenario, rho_X) %>%
  summarize(pct_favor_linear = mean(favor_linear), .groups = "drop")
